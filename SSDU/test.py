...
psnr = utils.getPSNR(np.array(all_ref_slices), np.array(all_recon_slices))

ssim_sum = 0
for i in range(len(all_ref_slices)):
    ssim = utils.getSSIM(all_ref_slices[i], all_recon_slices[i])
    ssim_sum += ssim
avg_ssim = ssim_sum / len(all_ref_slices)
print(f"\nPSNR: {psnr}\nSSIM: {avg_ssim}\n")
# print(psnr, avg_ssim)
        
slice_num = 0
fig, ax = plt.subplots(1, 3, figsize=(15, 10))
ax[0].imshow(np.abs(all_ref_slices[slice_num]), cmap='gray')
ax[0].set_title("GT Reference")
ax[0].set_axis_off()

ax[1].imshow(np.abs(all_recon_slices[slice_num]), cmap='gray')
ax[1].set_title("Reconstruction")
ax[1].set_axis_off()

ax[2].imshow(np.abs(all_input_slices[slice_num]), cmap='gray')
ax[2].set_title("Network Input")
ax[2].set_axis_off()
plt.show()
